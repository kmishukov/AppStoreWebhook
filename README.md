...
Create a directory and download the environment template:
...
Replace `latest` with a release such as `1.1.1` to pin the deployment to a
specific version.

The image includes a built-in health check. After startup, Docker reports the
container as `healthy` when the service is ready to receive notifications.

### Docker Compose

Alternatively, download the Compose file and start the same published image:
...
Set `APPSTORE_WEBHOOK_VERSION` in `.env` to a release such as `1.1.1` for a
